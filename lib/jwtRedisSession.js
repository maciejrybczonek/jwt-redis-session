...
module.exports = function(options){
	...
	options = _.defaults(options, {
		...
		useCookies: false,
		getToken: function(req, requestHeader, options) {
	...
	return function jwtRedisSession(req, res, next){
		...
		var token = options.getToken(req, requestHeader, options);
		...
		if(token){
			...
			jwt.verify(token, options.secret, function(error, decoded){
				...
				options.client.get(options.keyspace + (decoded.uid ? decoded.uid + ":" : "") + decoded.jti, function(err, session){
					if(err || !session)
						return next();
